...
(function () {
  ...
  // valueA/valueB só são usados se o elemento correspondente for um campo
  // de texto comum (select/dropdown sempre ignora o value e escolhe uma
  // opção real) — sem isso, um campo de texto que por engano caísse aqui
  // ficaria com o valor "undefined" escrito literalmente.
  async function fillDependentPair(elA, elB, valueA, valueB) {
    await fillField(elA, valueA);
    await sleep(80);
    await fillField(elB, valueB);
  }
