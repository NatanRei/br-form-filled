...
// vale já deixar como opção no MVP.
(function () {
  ...
  // Subconjunto pra MVP — trocar pela lista completa de municípios do IBGE
  ...
  function gerarCEP(formatado = true) {
    const str = digits(8).join('');
    return formatado ? `${str.slice(0, 5)}-${str.slice(5)}` : str;
  }
